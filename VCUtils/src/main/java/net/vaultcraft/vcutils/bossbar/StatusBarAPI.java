 ...
 * uses a trimmed down version of my own {@link net.vaultcraft.vcutils.bossbar.StatusBarAPI.PlayerMap} class to store the fake dragons.
 *
 * @author AmoebaMan
 *
 */
public class StatusBarAPI {
     ...
     * manually using {@link #removeStatusBar(org.bukkit.entity.Player)} when the player leaves the server
    ...
    /**
     * Removes the status bar for all players on the server.  See {@link #removeStatusBar(org.bukkit.entity.Player)}.
     */
    public static void removeAllStatusBars(){
        for(Player each : User.async_player_map.keySet())
            removeStatusBar(each);
    }

    /**
     * Sets the status bar for all players on the server.  See {@link #setStatusBar(org.bukkit.entity.Player, String, float)}.
     * @param text some text with 64 characters or less
     * @param percent a decimal percent in the range (0,1]
     */
     ...
     * uses {@link String} as the key type, using the player's name.
     * <br><br>
     * In addition to this memory-saving measure, this map also allows the contents to be accessed through
     * either the player's name or the player object itself, meaning no more hassle with {@link org.bukkit.entity.Player#getName()}
     * or {@link org.bukkit.Bukkit#getPlayer(String)} when you want to pull out of a map.
     *
     * @author AmoebaMan
     *
